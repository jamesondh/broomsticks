...
public class BroomstickApplet extends Applet implements Runnable {
  ...
  public void loadImages() {
    ...
    // sky     Idea: Put field and/or goal posts in the sky image?
  ...
  public boolean keyDown(Event e, int key) {
    ...
    switch (key) {
      ...
      case 'B':
        if (backToggle)
          ...
          backToggle = true;
        break;
      case e.LEFT:  players[1].left(); break;
      case e.RIGHT: players[1].right(); break;
      case e.UP:    players[1].up(); break;
      case e.ENTER: players[1].switchModel(); break;
      
      case 'e': case 'E':  players[0].up(); break;
      case 's': case 'S':  players[0].left(); break;
      case 'f': case 'F':  players[0].right(); break;
      case '1':            players[0].switchModel(); break;
      case 't': case 'T':  players[0].toggleRobot(); break;
    ...
    if (teams) {
      ...
      if (key == 'l' || key == 'L')
        players[3].right();
      if (key == '7' || key == '&')
        players[3].switchModel();
    }

    // robot control
    if (key == 't' || key == 'T') {
      if (players[0].isRobot) {
        players[0].isRobot = false;
    ...
    if (key == 'p' || key == 'P') {
      if (players[2].isRobot) {
        players[2].isRobot = false;
        players[2].velocityX = 0;
      }
      else
        players[2].isRobot = true;
      paint(getGraphics());
    }
    if (key == e.INSERT) {
      if (players[3].isRobot) {
        players[3].isRobot = false;
        players[3].velocityX = 0;
      }
      else
        players[3].isRobot = true;
      paint(getGraphics());
    }

    if (players[0].isRobot && (key == 's' || key == 'S')) {
      players[0].dumber();
      drawSkill(getGraphics());    
    }

    if (players[0].isRobot && (key == 'f' || key == 'F')) {
      players[0].smarter();
      drawSkill(getGraphics());
    }
  ...
  public void drawScene(Graphics g) {
    if (!tracker.checkAll(true)) {
      ...
      if (teams) {
        ...
        g.drawString("NEW! Press P for computer control", 100, 290);
        g.drawString("During computer control, use S and F to adjust skill", 100, 305);
        g.setColor(Color.black);
      }
  ...
  public void drawSkill(Graphics g, int x, int y, int smart) {
    g.setColor(getBackground());
    g.fillRect(x, y, 34, 10);
    g.setColor(Color.red);
    g.fillRect(x, y, 35-smart, 10);
    g.setColor(Color.black);
    g.drawRect(x, y, 34, 10);
  }
  ...
  public void paint(Graphics g) {
    ...
    if (players[0].isRobot) {
      g.setColor(Color.red);
      g.drawString("S F T", 50, height-5);
      drawSkill(g, 10, height-15, players[0].smart);
    }
    else
      g.drawString("E S F 1 and T", 50, height-5);
    g.drawString("arrow-keys and ENTER", width-150, height-5);
    if (teams) {
      if (players[2].isRobot) {
        g.setColor(Color.red);
        g.drawString("J L P", 50, height-5);
        drawSkill(g, midW-100, height-15, players[2].smart);
      }
      else
        g.drawString("I J L 7 and P", midW - 100, height-5);
      if (players[3].isRobot) {
        g.setColor(Color.red);
        g.drawString("Del PgDn PgUp", midW, height-5);
        drawSkill(g, midW, height-15, players[3].smart);
      }
      else
        g.drawString("Home Del PgDn Ins and PgUp", midW, height-5);
    }
    else
      g.drawString("b to change background", midW-65, height-5);
  }
...
class Person extends FlyingObject {
  ...
  public void handleKeyEvents(Event e, int key) {
    if (isRobot) {
      if (key == leftKey)
        dumber();
      if (key == rightKey)
  ...
  public void drawInfo(Graphics g) {
    g.setColor(getBackground());
    g.fillRect(infoX, infoY, 150, 20);
    if (!isRobot) {
      g.setColor(Color.black);
      g.drawString((char) upKey + " " + (char) leftKey + " " + (char) rightKey
                 + (char) modelKey + " " + robotKey, infoX, infoY+10);
    }
    else {
      g.setColor(Color.red);
      ...
      g.drawStrimg((char) leftKey + " " + (char) rightKey + " "

...
void Person::move() {
  if (isRobot && target) {
    ...
    if (now - lastMoveTime >= 100) {
      lastMoveTime = now;
      //int choices = (int) (smart*25.0/etime); // base on 40 fps
...
void Person::erase() {
  ...
  //graphics->eraseBox((int) oldx, (int) oldy, 39, 39);
}
...
#if 0
    if (side) 
      graphics->text(infoX+85+35+10, infoY, "SHIFT");
    else
      graphics->text(infoX+85+35+10, infoY, "S F     p for 2 player");
#else
    if (upKey == SDLK_UP)
...
#if 0
    if (side)
      graphics->text(infoX, infoY, "arrow-keys ENTER");
    else
      graphics->text(infoX, infoY, "E X S F 1            p for 1 player");
#else
    if (upKey == SDLK_UP)
